...
license: Apache-2.0
compatibility: Requires git, gh CLI, and internet access to github.com
metadata:
...
grep -rh "tektoncd/plumbing" .github/workflows/ | grep -oP '@[0-9a-f]{40}' | sort -u
```
...
```bash
OLD=$(grep -rh "tektoncd/plumbing" .github/workflows/ | grep -oP '@[0-9a-f]{40}' | sort -u | head -1)
...
for f in $(grep -rln "tektoncd/plumbing" .github/workflows/); do
  sed -i "s|tektoncd/plumbing/.github/workflows/\(.*\)@${OLD}|tektoncd/plumbing/.github/workflows/\1@${NEW}|g" "$f"
  echo "Updated $f"
done
```
